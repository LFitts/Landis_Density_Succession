...
ageClass <- function(ecoregion, spcd, diameter)
{
  ...
  if (!exists('landGrow'))
  {
    landGrow <- read.table('simulations/s3_s4/Ecoregion_diameter_table_orig.txt', skip=4, col.names=c('ECOREGION','SPECIES','AGE','DIAMETER')) ##########update subplots when needed
  }
...
landGrow <- read.table('simulations/s3_s4/Ecoregion_diameter_table_orig.txt', skip=4, col.names=c('ECOREGION','SPECIES','AGE','DIAMETER')) ###########update subplot location when needed
#'
MV_KEY <- data.frame()
PLOTMAPVALUE <- 1
outFile = file('simulations/s3_s4/Initial_Community_orig.txt', 'w') ###### update output location when needed
...
close(outFile)
write_csv(MV_KEY, 'simulations/s3_s4/output/MAPVALUE_KEY_orig.csv') ###########update output when needed
#' 
#' ##################################################################################
# 2. Maps: Subplots with ecoregions ----
#'################################################################################### 
#'
MV_KEY<-read_csv('simulations/s3_s4/output/MAPVALUE_KEY_orig.csv') ################update subplot location
...
#
#' Save the files
writeRaster(ecoregion_map, "simulations/s3_s4/ecoregion_orig.img", NAflag=-9999, overwrite=T, datatype='INT2S') ############update output location
#'
#plot(ecoregion_map)
#'
writeRaster(initial_communities_map, "simulations/s3_s4/initialcommunity_orig.img", NAflag=-9999, overwrite=T, datatype='INT2S') ############## update output location
...
MV_KEY<-read.csv('simulations/s3_s4/output/MAPVALUE_KEY_orig.csv') ############# update subplot location
...
ageClass <- function(ecoregion, spcd, diameter)
{
  ...
  if (!exists('landGrow'))
  {
    landGrow <- read.table('simulatios/s3_s4/Ecoregion_diameter_table_orig.txt.txt', skip=4, col.names=c('ECOREGION','SPECIES','AGE','DIAMETER'))  ############ update subplot location
  }
  if (diameter <= min(landGrow[(landGrow$SPECIES == spcd) & (landGrow$ECOREGION == ecoregion), 'DIAMETER']))
  {return(min(landGrow[(landGrow$SPECIES == spcd) & (landGrow$ECOREGION == ecoregion), 'AGE']))}
  if (diameter >= max(landGrow[(landGrow$SPECIES == spcd) & (landGrow$ECOREGION == ecoregion), 'DIAMETER']))
...
#' Write age cohort and number of trees out to LANDIS initial community file
#' 
outFile <-  file('simulations/s3_s4/land-use_orig.txt', 'w') ############### update output
#'
#'
...
close(outFile)
write_csv(luKEY, 'simulations/s3_s4/output/landuse_key_orig.csv') ################# update output
#'
#' Create land-use maps
#' 
MV_KEY <- read_csv('simulations/s3_s4/output/MAPVALUE_KEY_orig.csv') ###############update subplot
MV_KEY <- MV_KEY %>% rowwise() %>% mutate(KEY = paste(str_split(PLT_KEY, '_', simplify=T)[,1:3], collapse='_'))
MV_KEY <- MV_KEY %>% mutate(SUBKEY = str_c(KEY, '_', str_split(PLT_KEY, '_', simplify=T)[,5]))
#'
...
for (ts in 0:20)
{
  writeRaster(lu_map, paste0("simulations/s3_s4/landuse-orig", ts, ".img"), NAflag=-9999, overwrite=T, datatype='INT2S') ################## update output
}
...
for (ts in unique(mapKey$TIMESTEP)[order(unique(mapKey$TIMESTEP))])
{
  ...
  writeRaster(sub_map, paste0("simulations/s3_s4/landuse-orig", ts, ".img"), NAflag=-9999, overwrite=T, datatype='INT2S') ############ update output location
}
