...
#' 
#' With the tree table:
wi.st <- wiTB$TREE %>% filter(DIA < 5 & STATUSCD == 1 & SPCD %in% spcds) %>%  dplyr::select(CN, PLT_CN, PREV_TRE_CN, INVYR,CYCLE, PLOT, SUBP, TREE, SPCD, DIA)
#' With the plot table (to get the ecoregions)
wi.sp <- wiTB$PLOT %>%  dplyr::select(PLT_CN, ECOSUBCD)
wi.sp$ECO_PROVINCE<- substr(wi.sp$ECOSUBCD, start=2, stop=5) #Leave only the strings that correspond to the ecological province (from 2 to 5). Note that there is a blanc space at the beginning of the ECOSUBCD column from the FIA database
#' 
#' Now keep only the PLT_CN and the ecological province
#' 
wi.sp<-wi.sp %>%  dplyr::select(PLT_CN, ECO_PROVINCE)
#'
...
wi.tg <- wi.st %>% filter(CYCLE == 9) %>% 
  left_join(wi.st %>% filter(CYCLE == 8) %>%  dplyr::select(CN, PREV_TRE_CN, DIA), by = c('PREV_TRE_CN' = 'CN')) 
#'
...
#smallDiaSlow <- c(12, 95,701, 746, 762) ####
...
for(i in 1:length(sp_eco_listWI)){
  ...
  while(max(smallGrowthMD$DIA) < 5){
    ...
    subTB <- smallWorkingTB %>% filter(sizeClass <= max(smallGrowthMD$DIA)) %>% ungroup() %>%  dplyr::select(DIA_GROW) 
  ...
  while(max(growthMD$DIA) < maxtable$DIA_P99[which(maxtable$KEY==tempkey)]){
    if (max(growthMD$DIA) < min(workingTB$sizeClass, na.rm = T))
    {
      ...
      subTB <- workingTB %>% filter(sizeClass <= max(growthMD$DIA)) %>%  dplyr::select(DIA_GROW)
      diaGR <- subTB %>% slice(nrow(subTB))
    }
  ...
  dia_list[[tempkey]] <- age_dia %>%  dplyr::select(ECO_PROVINCE, Name, AGE, DIA)
  print(tempkey)
...
ecoSp <- expand.grid(unique(wiTB$PLOT$ECO_PROVINCE), unique(c(abiebals_smallDiaSlow, picemari_smallDiaSlow, ostrvirg_smallDiaSlow, poputrem_smallDiaSlow, prunsero_smallDiaSlow, thujocci_smallDiaBoost, queralba_smallDiaBoost, picemari_largeDiaSlow, pinubank_largeDiaSlow, poputrem_largeDiaSlow, queralba_largeDiaBoost, thujocci_largeDiaBoost, tsugcana_largeDiaBoost)), stringsAsFactors = F) %>%
  mutate(SPECOKEY = str_c(Var1, Var2, sep = '_')) %>%  dplyr::select(SPECOKEY)
